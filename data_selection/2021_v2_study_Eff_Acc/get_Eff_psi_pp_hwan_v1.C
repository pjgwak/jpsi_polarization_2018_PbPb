...
void get_Eff_psi_pp_hwan_v1(
  ...
  if(state==2) inputMC1 = "/work2/Oniatree/Psi2S/OniatreeMC_BtoPsi2S_pThat-2_TuneCP5-EvtGen_HydjetDrumMB_5p02TeV_pythia8.root";	//PbPb_non prompt
  ...
  //for(int iev=0; iev<300000 ; ++iev)
  {
    ...
    for(int igen = 0; igen<Gen_QQ_size; igen++){
		...
		if(isPtWeight && fabs(JP_Gen->Rapidity()) < 2.4 && JP_Gen->Pt() > 6.5 ) pt_weight = fptw1->Eval(JP_Gen->Pt());
		if(isPtWeight && fabs(JP_Gen->Rapidity()) > 1.6 && JP_Gen->Pt() < 6.5 ) pt_weight = fptw2->Eval(JP_Gen->Pt());
        ...
        //if(pt_weight < 0) pt_weight = fabs(pt_weight);
...
//         cout << "TnP ERROR !!!! ::: No matched L2 filter1 " << endl;
      ...
      if(isPtWeight && fabs(JP_Reco->Rapidity()) < 2.4 && JP_Reco->Pt() > 6.5) pt_weight = fptw1->Eval(JP_Reco->Pt());
      if(isPtWeight && fabs(JP_Reco->Rapidity()) > 1.6 && JP_Reco->Pt() < 6.5) pt_weight = fptw2->Eval(JP_Reco->Pt());
...
//draw same
  ...
  hpt_eff_6->Draw("same");
  legend->AddEntry("hpt_eff_1","|y|: 0-2.4","lep");
  legend->AddEntry("hpt_eff_2","|y|: 0-0.6","lep");
  legend->AddEntry("hpt_eff_3","|y|: 0.0-1.2","lep");
  legend->AddEntry("hpt_eff_4","|y|: 1.2-1.8","lep");
  legend->AddEntry("hpt_eff_5","|y|: 1.2-1.6","lep");
  legend->AddEntry("hpt_eff_6","|y|: 1.6-2.4","lep");
  legend->SetBorderSize( 0);
  legend->Draw("E");
  cpt_eff->SaveAs("Eff_pt_noweight_230307.png");
  ...
  cy_eff->SaveAs("Eff_absy_noweight_230307.png");

  //Save efficiency files for later use.

  hpt_eff_1 ->SetName(Form("mc_eff_vs_pt_TnP%d_PtW%d_absy0_2p4",isTnP, isPtWeight));
  hpt_eff_2 ->SetName(Form("mc_eff_vs_pt_TnP%d_PtW%d_absy0_0p6",isTnP, isPtWeight));
  hpt_eff_3 ->SetName(Form("mc_eff_vs_pt_TnP%d_PtW%d_absy0_1p2",isTnP, isPtWeight));
  hpt_eff_4 ->SetName(Form("mc_eff_vs_pt_TnP%d_PtW%d_absy1p2_1p6",isTnP, isPtWeight));
  hpt_eff_5 ->SetName(Form("mc_eff_vs_pt_TnP%d_PtW%d_absy1p6_1p8",isTnP, isPtWeight));
  hpt_eff_6 ->SetName(Form("mc_eff_vs_pt_TnP%d_PtW%d_absy1p8_2p4",isTnP, isPtWeight));
  ...
  TString outFileName = Form("mc_eff_vs_pt_rap_prompt_pp_psi2s_PtW%d_tnp%d_20230321.root",isPtWeight,isTnP);
