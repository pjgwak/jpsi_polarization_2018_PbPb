...
void eff_pb(string data_label_ = "all_event", int nevt = -1, int MCtype = 1, bool isTnP = true, bool isPtWeight = true)
{
    ...
    for (int iev = 0; iev < nevt; ++iev)
    {
        ...
        for (Int_t irqq = 0; irqq < Reco_QQ_size; ++irqq)
        {
            ...
            {
                all_y_lab_num->Fill(Centrality, cos_lab_, phi_lab_, pt_reco, weight * pt_weight);
                all_y_hx_num->Fill(Centrality, cos_hx_, phi_hx_, pt_reco, weight * pt_weight);
                all_y_cs_num->Fill(Centrality, cos_cs_, phi_cs_, pt_reco, weight * pt_weight);
                all_y_ep_num->Fill(Centrality, cos_ep_, phi_ep_, pt_reco, weight * pt_weight);
            }

            // fwd vs mid
            if (Rapidity_reco > 1.6 && Rapidity_reco < 2.4 && pt_reco > 3 && pt_reco < 50)
            {
                fwd_lab_num->Fill(Centrality, cos_lab_, phi_lab_, pt_reco, weight * pt_weight);
                fwd_hx_num->Fill(Centrality, cos_hx_, phi_hx_, pt_reco, weight * pt_weight);
                fwd_cs_num->Fill(Centrality, cos_cs_, phi_cs_, pt_reco, weight * pt_weight);
                fwd_ep_num->Fill(Centrality, cos_ep_, phi_ep_, pt_reco, weight * pt_weight);
            }
            else if (Rapidity_reco < 1.6 && pt_reco > 6.5 && pt_reco < 50)
            {
                mid_lab_num->Fill(Centrality, cos_lab_, phi_lab_, pt_reco, weight * pt_weight);
                mid_hx_num->Fill(Centrality, cos_hx_, phi_hx_, pt_reco, weight * pt_weight);
                mid_cs_num->Fill(Centrality, cos_cs_, phi_cs_, pt_reco, weight * pt_weight);
                mid_ep_num->Fill(Centrality, cos_ep_, phi_ep_, pt_reco, weight * pt_weight);
            }
        } // end of reco loop
    } // end of main loop
