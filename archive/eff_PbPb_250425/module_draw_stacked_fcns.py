...
def get_1d_correction(hists, axis, save_name):
    # to avoid potential memory leak when using 4d-hist
    hists[0].SetName('h4_temp_name_num' + save_name)
    hists[1].SetName('h4_temp_name_den' + save_name)

    h_num = hists[0].Projection(axis)
    h_den = hists[1].Projection(axis)
...
def draw_comparison_plot(hists1, hists2, hists3, axis, title='', x_title='', y_title='', save_name=''):
    hist1 = get_1d_correction(hists1, axis, save_name+'fwd')
    hist2 = get_1d_correction(hists2, axis, save_name+'mid')
    hist3 = get_1d_correction(hists3, axis, save_name+'all_y')
    ...
    # legend
    legend = TLegend(0.54, 0.76, 0.93, 0.9)
    legend.SetBorderSize(0) # no boarder
    legend.SetFillColorAlpha(ROOT.kWhite, 0) # transperency
    legend.SetTextSize(0.03) # text size
    legend.AddEntry(hist1, '1.6 < |y| < 2.4, p_{T} > 3', 'ep')
    legend.AddEntry(hist2, '|y| < 1.6, p_{T} > 6.5', 'ep')
    legend.AddEntry(hist3, '|y| < 2.4, p_{T} > 6.5', 'ep')
    ...
    hist1.SetTitle(f'{title}; {x_title}; {y_title}')
    # hist1.GetXaxis().SetTitle(x_title)
    # hist1.GetYaxis().SetTitle(y_title)
