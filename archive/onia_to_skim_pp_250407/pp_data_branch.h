...
// Tue Apr  8 20:01:49 2025 by ROOT version 6.32.02
// from TTree myTree/My TTree of dimuons
// found on file: /work/pjgwak/oniatree_PbPb_data_AOD/pp_data/OniaTree_DoubleMuPD_pp2017_5p02TeV_EOY_merged.root
//////////////////////////////////////////////////////////

#ifndef pp_data_branch_h
#define pp_data_branch_h
...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   Short_t         Reco_QQ_type[14];   //[Reco_QQ_size]
   Short_t         Reco_QQ_sign[14];   //[Reco_QQ_size]
   TClonesArray    *Reco_QQ_4mom;
   Short_t         Reco_QQ_mupl_idx[14];   //[Reco_QQ_size]
   Short_t         Reco_QQ_mumi_idx[14];   //[Reco_QQ_size]
   ULong64_t       Reco_QQ_trig[14];   //[Reco_QQ_size]
   Bool_t          Reco_QQ_isCowboy[14];   //[Reco_QQ_size]
   Float_t         Reco_QQ_ctau[14];   //[Reco_QQ_size]
   Float_t         Reco_QQ_ctauErr[14];   //[Reco_QQ_size]
   Float_t         Reco_QQ_cosAlpha[14];   //[Reco_QQ_size]
   Float_t         Reco_QQ_ctau3D[14];   //[Reco_QQ_size]
   Float_t         Reco_QQ_ctauErr3D[14];   //[Reco_QQ_size]
   Float_t         Reco_QQ_cosAlpha3D[14];   //[Reco_QQ_size]
   Float_t         Reco_QQ_VtxProb[14];   //[Reco_QQ_size]
   Float_t         Reco_QQ_dca[14];   //[Reco_QQ_size]
   Float_t         Reco_QQ_MassErr[14];   //[Reco_QQ_size]
   TClonesArray    *Reco_QQ_vtx;
   Short_t         Reco_mu_size;
   Short_t         Reco_mu_type[6];   //[Reco_mu_size]
   Int_t           Reco_mu_SelectionType[6];   //[Reco_mu_size]
   Short_t         Reco_mu_charge[6];   //[Reco_mu_size]
   TClonesArray    *Reco_mu_4mom;
   ULong64_t       Reco_mu_trig[6];   //[Reco_mu_size]
   Bool_t          Reco_mu_isPF[6];   //[Reco_mu_size]
   Bool_t          Reco_mu_InTightAcc[6];   //[Reco_mu_size]
   Bool_t          Reco_mu_InLooseAcc[6];   //[Reco_mu_size]
   ...
   pp_data_branch(TTree *tree=0);
   virtual ~pp_data_branch();
...
#ifdef pp_data_branch_cxx
pp_data_branch::pp_data_branch(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("/work/pjgwak/oniatree_PbPb_data_AOD/pp_data/OniaTree_DoubleMuPD_pp2017_5p02TeV_EOY_merged.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("/work/pjgwak/oniatree_PbPb_data_AOD/pp_data/OniaTree_DoubleMuPD_pp2017_5p02TeV_EOY_merged.root");
      }
      TDirectory * dir = (TDirectory*)f->Get("/work/pjgwak/oniatree_PbPb_data_AOD/pp_data/OniaTree_DoubleMuPD_pp2017_5p02TeV_EOY_merged.root:/hionia");
...
pp_data_branch::~pp_data_branch()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t pp_data_branch::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t pp_data_branch::LoadTree(Long64_t entry)
{
...
void pp_data_branch::Init(TTree *tree)
{
...
void pp_data_branch::Show(Long64_t entry)
{
...
Int_t pp_data_branch::Cut(Long64_t entry)
{
...
#endif // #ifdef pp_data_branch_cxx
