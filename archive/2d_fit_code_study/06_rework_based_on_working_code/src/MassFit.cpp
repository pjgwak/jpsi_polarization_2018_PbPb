...
//   // massLow, massHigh: common header
...
//   // SIGNAL
//   RooRealVar mean("m_{J/#Psi}", "mean of the signal gaussian mass PDF", pdgMass.JPsi, pdgMass.JPsi - 0.05, pdgMass.JPsi + 0.05);
//   RooRealVar *x_A = new RooRealVar("x_A", "sigma ratio ", x_init);
//   RooRealVar sigma_1_A("sigma_1_A", "width/sigma of the signal gaussian mass PDF", sigma_1_init, sigma_1_init - 0.01, sigma_1_init + 0.01);
...
//   // Double_t NJpsi_limit = 2.0e+08;

//   RooRealVar *N_Jpsi = new RooRealVar("N_Jpsi", "inclusive Jpsi signals", 100000, 1, 1000000);
//   RooRealVar *N_Bkg = new RooRealVar("N_Bkg", "fraction of component 1 in bkg", 100000, 1, 1000000);
...
//   RooFitResult *fitMass = ws->pdf("pdfMASS_Tot")->fitTo(*dsAB, Save(), Timer(kTRUE), Extended(kTRUE), SumW2Error(isWeighted), NumCPU(nCPU), Strategy(2));
//   cout << endl
//        << "********* Finished Mass Dist. Fit **************" << endl
//        << endl;
//   // ws->pdf("pdfMASS_Tot")->plotOn(myPlot2_A, VisualizeError(*fitMass, 1), FillColor(kOrange));
//   ws->pdf("pdfMASS_Tot")->plotOn(myPlot2_A, Name("pdfMASS_Tot"), LineColor(kBlack));
//   ws->pdf("pdfMASS_Tot")->plotOn(myPlot2_A, Components(RooArgSet(*pdfMASS_bkg, *cb_1_A)), LineColor(44), LineWidth(2), LineStyle(kDashed));
//   ws->pdf("pdfMASS_Tot")->plotOn(myPlot2_A, Components(RooArgSet(*pdfMASS_bkg, *cb_2_A)), LineColor(8), LineWidth(2), LineStyle(kDashed));
//   // model.plotOn(myPlot2_A,Name("model"), LineColor(kBlack));
