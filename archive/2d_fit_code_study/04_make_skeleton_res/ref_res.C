...
// RooFitResult *perfromResFit(RooWorkspace *ws, double &ctauResMin, double &ctauResMax);
// void drawResPlot(RooWorkspace * ws, RooFitResult * fitRes, double ctauResMin, double ctauResMax, const std::string outputPath);
...
RooWorkspace* setupWorkspace(TFile *f_mass, TFile *f_err)
{
  // RooAddPdf *massModel = (RooAddPdf *)f_mass->Get("massModel");
  // RooDataSet *ds_errSig = (RooDataSet *)f_err->Get("ds_errSig");

  // RooWorkspace *ws = new RooWorkspace("ws_res"); // should be ws_res
  // ws->import(*massModel);
  // ws->import(*ds_errSig);

  // // nominal Res range cut.
  // RooDataSet *ctauResCutDS = (RooDataSet *)ds_sig_err->reduce("ctau3DRes<0");
...
void processDataset() {
  ...
  // RooDataSet *ctauResCutDS = (RooDataSet *)ds_errSig->reduce(RooArgSet(*(ws_res->var("ctau3DRes")), *(ws_res->var("ctau3D")), *(ws_res->var("ctau3DErr"))), Form("ctau3DRes<0&&ctau3DErr>%f&&ctau3DErr<%f", ctauErrMin, ctauErrMax));
  // ctauResCutDS->SetName("ctauResCutDS");
  ...
  // TH1D *h_tmp = (TH1D *)ws_res->data("ds_errSig")->createHistogram(("h_tmp"), *ws_res->var("ctau3DRes"), Binning(nCtauResBins, ctauResLow, ctauResHigh));
...
void buildResModel(RooWorkspace *ws, int nGauss)
{
  ...
  // // use config later
  // ws->factory("ctauRes_mean[0.]"); // fix mean of gauss
  ...
  // RooAbsPdf *ctauResModel = ctauResModel = new RooAddPdf("gaus_res_model", "gaus_res_model", *ws_res->pdf("GaussModelCOND_ctauRes"), *ws_res->var("nSig"));
