...
def draw_2d_hist_maps(h4_num, h4_den, axis1, axis2, legend1='', legend2='', x_title='', y_title='', save_name=''):
    # # copy rootlogon style
    # saved_style = ROOT.TStyle("saved_style", "Saved Style from before 2D Specific Plot")
    # saved_style.Copy(ROOT.gStyle)

    # # overlap local 2D plot setting - ROOT's basic setting
    # ROOT.gStyle.SetPalette(ROOT.kViridis)
    # ROOT.gStyle.SetNumberContours(20)
    # ROOT.gStyle.SetLabelSize(0.035, "Z")
    # ROOT.gStyle.SetLabelFont(42, "Z")
    # ROOT.gStyle.SetLabelOffset(0.004999999888241291, "Z")
    # ROOT.gStyle.SetTitleSize(0.035, "Z")
    # ROOT.gStyle.SetTitleFont(42, "Z")
    # ROOT.gStyle.SetTitleOffset(1.0, "Z")
    # ROOT.gStyle.SetPadLeftMargin(0.10000000149011612)
    # ROOT.gStyle.SetPadRightMargin(0.10000000149011612)
    # ROOT.gStyle.SetPadBottomMargin(0.10000000149011612)
    # ROOT.gStyle.SetPadTopMargin(0.10000000149011612)
    ...
    c_hist.cd(3)
    h_cor = h_num.Clone("h_cor")
    h_cor.Divide(h_den) # set 0

    # for i in range(1, h_num.GetNbinsX() + 1):
    #     for j in range(1, h_num.GetNbinsY() + 1):
    #         A = h_num.GetBinContent(i, j)
    #         B = h_den.GetBinContent(i, j)
    #         eA = h_num.GetBinError(i, j)
    #         eB = h_den.GetBinError(i, j)

    #         if B != 0:
    #             ratio = A / B
    #             error = ((eA / B)**2 + (A * eB / B**2)**2) ** 0.5
    #             h_cor.SetBinContent(i, j, ratio)
    #             h_cor.SetBinError(i, j, error)
    #         else:
    #             h_cor.SetBinContent(i, j, 0)
    #             h_cor.SetBinError(i, j, 0)
    ...
    h_cor.SetMaximum(1)
    h_cor.SetMinimum(0.0000001) # don't draw zero bin
    h_cor.GetXaxis().CenterTitle(True)
    h_cor.GetYaxis().CenterTitle(True)
    ...
    for ix in range(1, h2_mask.GetNbinsX() + 1):
        for iy in range(1, h2_mask.GetNbinsY() + 1):
            if h2_mask.GetBinContent(ix, iy) > 0:
                ...
                print('Correction > 1')
                print(save_name)
                print(f'Bin at ({ix}, {iy})')
                print(f'Numerator: {h_num.GetBinContent(ix, iy)} +- {h_num.GetBinError(ix, iy)}')
                print(f'Denominaotr: {h_den.GetBinContent(ix, iy)} +- {h_den.GetBinError(ix, iy)}')
                print(f'Correction: {h_cor.GetBinContent(ix, iy)} +- {h_cor.GetBinError(ix, iy)}')
    ...
    # # restore rootlogon style
    # ROOT.gStyle.Copy(saved_style)
...
#                 print(save_name)
#                 print(f'Bin at ({ix}, {iy})')
#                 print(f'{h_cor.GetBinContent(ix, iy)} +- {h_cor.GetBinError(ix, iy)}')
...
def draw_cos_phi_maps(h4_num, h4_den, pt_bins, axis_pt, axis1, axis2, legend1='', legend2='', x_title='', y_title='', save_name=''):
    ...
    for idx in range(1, n_bins):
        ...
        draw_2d_hist_maps(h4_num=h4_num, h4_den=h4_den, axis1=axis1, axis2=axis2, legend1='', legend2='', x_title=x_title, y_title=y_title, save_name=save_name + "_" + name_tag)
