...
#include "../../headers/tnp_weight_pp.h"
...
void eff_pp(string data_label_ = "all_event", int nevt = -1, int MCtype = 1, bool isTnP = true, bool isPtWeight = true)
{
    TStopwatch *t = new TStopwatch;
    t->Start();
    cout << "\n=================================\n";
    cout << "\n Start pp Eff Calculation\n";
    cout << "\n=================================\n\n";
    ...
    {
        // pp MC prompt
        TString filename = Form("/disk1/Oniatree/Jpsi/OniaTreeMC_Jpsi_Pythia8_nonUL_5p02TeV_merged.root");
    ...
    vector<double> cos_bin = {-1.0, -0.9, -0.75, -0.6, -0.45, -0.3, -0.15, 0.0, 0.15, 0.3, 0.45, 0.6, 0.75, 0.9, 1.0};
    vector<double> phi_bin = {-3.15, -3.0, -2.4, -1.8, -1.2, -0.6, 0.0, 0.6, 1.2, 1.8, 2.4, 3.0, 3.15};
    ...
    for (int iev = 0; iev < nevt; ++iev)
    {
        ...
        for (Int_t irqq = 0; irqq < Reco_QQ_size; ++irqq)
        {
            ...
            if (Reco_mu_whichGen[Reco_QQ_mupl_idx[irqq]] == -1) // 이건 PbPb에만 있을 것 같은데. pp MC는 mixing을 안 하니까.
                continue;
    ...
    // ===== save output ===== //
    TString outFileName = Form("roots/eff_pp_Jpsi_PtW%d_tnp%d_%s.root", isPtWeight, isTnP, date_label.Data());
    if (MCtype == 2)
        outFileName = Form("roots/eff_pp_Jpsi_PtW%d_tnp%d_%s.root", isPtWeight, isTnP, date_label.Data());
