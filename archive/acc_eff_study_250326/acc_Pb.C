...
void acc_Pb(string data_label_ = "all_event", int nevt = -1, int MCtype = 1, int cLow = 0, int cHigh = 180, bool isTnP = true, bool isPtWeight = true)
{
    ...
    // 3d histograms
    // ===== fwd ===== //
    auto fwd_ep_num = std::make_shared<TH3D>("fwd_ep_num", "", n_cos, &cos_bin[0], n_phi, &phi_bin[0], n_fwd_pt, &fwd_pt_bin[0]);
    auto fwd_ep_den = std::make_shared<TH3D>("fwd_ep_den", "", n_cos, &cos_bin[0], n_phi, &phi_bin[0], n_fwd_pt, &fwd_pt_bin[0]);
    ...
    // ===== mid ===== //
    auto mid_ep_num = std::make_shared<TH3D>("mid_ep_num", "", n_cos, &cos_bin[0], n_phi, &phi_bin[0], n_mid_pt, &mid_pt_bin[0]);
    auto mid_ep_den = std::make_shared<TH3D>("mid_ep_den", "", n_cos, &cos_bin[0], n_phi, &phi_bin[0], n_mid_pt, &mid_pt_bin[0]);
    ...
    for (int iev = 0; iev < nevt; ++iev)
    {
        ...
        for (int igen = 0; igen < Gen_QQ_size; igen++)
        {
            ...
            if (Rapidity_gen > 1.6 && Rapidity_gen < 2.4)
            {
                ...
                fwd_ep_den->Fill(cos_ep_, phi_ep_, JP_Gen->Pt(), weight * pt_weight);
                fwd_hx_den->Fill(cos_hx_, phi_hx_, JP_Gen->Pt(), weight * pt_weight);
                fwd_cs_den->Fill(cos_cs_, phi_cs_, JP_Gen->Pt(), weight * pt_weight);
            }
            else
            {
                mid_ep_den->Fill(cos_ep_, phi_ep_, JP_Gen->Pt(), weight * pt_weight);
            ...
            if (Rapidity_gen > 1.6 && Rapidity_gen < 2.4)
            {
                fwd_ep_num->Fill(cos_ep_, phi_ep_, JP_Gen->Pt(), weight * pt_weight);
                fwd_hx_num->Fill(cos_hx_, phi_hx_, JP_Gen->Pt(), weight * pt_weight);
                fwd_cs_num->Fill(cos_cs_, phi_cs_, JP_Gen->Pt(), weight * pt_weight);
            }
            else
            {
                mid_ep_num->Fill(cos_ep_, phi_ep_, JP_Gen->Pt(), weight * pt_weight);
