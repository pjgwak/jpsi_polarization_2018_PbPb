...
void EventLoop::initialize()
{
  ...
  if (useRun2)
    m_data = new DataRun2(m_chain, isMC, isGenOnly, isEP, useRun2);
  else
    m_data = new DataRun3(m_chain, isMC, isGenOnly, isEP, useRun2);
...
void EventLoop::execute()
{
  ...
  for (Long64_t ir = 0; ir < nEvent; ++ir)
  {
    ...
    if (useRun2) {
        data_run2 = dynamic_cast<DataRun2*>(m_data);
        recoQQSize = static_cast<Long64_t>(data_run2->Reco_QQ_size);
    } else {
        data_run3 = dynamic_cast<DataRun3*>(m_data);
...
void EventLoop::executeGen()
{
  ...
  for (Long64_t ir = 0; ir < nEvent; ++ir)
  {
    ...
    // prepare variables
    Long64_t genQQSize = 0;
    if (useRun2)
    {
      DataRun2 *data_run2 = dynamic_cast<DataRun2 *>(m_data);
      genQQSize = static_cast<Long64_t>(data_run2->Gen_QQ_size);
    }
    else
    {
      DataRun3 *data_run3 = dynamic_cast<DataRun3 *>(m_data);
      genQQSize = static_cast<Long64_t>(data_run3->Gen_QQ_size);
    }
