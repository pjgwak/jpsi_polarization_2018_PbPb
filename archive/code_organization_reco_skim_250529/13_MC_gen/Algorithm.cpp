...
void Algorithm::standByFillingGen(Long64_t igqq) {
  Data &data = *m_data;
  ...
  // Gen variables
  mass[nDimu] = data.JP_Gen->M();
  phi[nDimu] = data.JP_Gen->Phi();
  phi1[nDimu] = data.mupl_Gen->Phi();
  phi2[nDimu] = data.mumi_Gen->Phi();
  y[nDimu] = data.JP_Gen->Rapidity();
  pt[nDimu] = data.JP_Gen->Pt();
  pt1[nDimu] = data.mupl_Gen->Pt();
  pt2[nDimu] = data.mumi_Gen->Pt();
  eta[nDimu] = data.JP_Gen->Eta();
  eta1[nDimu] = data.mupl_Gen->Eta();
  eta2[nDimu] = data.mumi_Gen->Eta();
  // cos_theta[nDimu] = data.JP_Gen->CosTheta();
  // cos_theta1[nDimu] = data.mupl_Gen->CosTheta();
  ctau3D[nDimu] = data.Gen_QQ_ctau3D[igqq];
  cosHX[nDimu] = muplHX.CosTheta();
  phiHX[nDimu] = muplHX.Phi();
  cosCS[nDimu] = muplCS.CosTheta();
  phiCS[nDimu] = muplCS.Phi();
