...
namespace ROOT {
   static TClass *Cut_Dictionary();
   static void Cut_TClassManip(TClass*);
   static void *new_Cut(void *p = nullptr);
   static void *newArray_Cut(Long_t size, void *p);
   static void delete_Cut(void *p);
   static void deleteArray_Cut(void *p);
   static void destruct_Cut(void *p);

   // Function generating the singleton type initializer
   static TGenericClassInfo *GenerateInitInstanceLocal(const ::Cut*)
   {
      ::Cut *ptr = nullptr;
      static ::TVirtualIsAProxy* isa_proxy = new ::TIsAProxy(typeid(::Cut));
      static ::ROOT::TGenericClassInfo 
         instance("Cut", "Cut.h", 3,
                  typeid(::Cut), ::ROOT::Internal::DefineBehavior(ptr, ptr),
                  &Cut_Dictionary, isa_proxy, 4,
                  sizeof(::Cut) );
      instance.SetNew(&new_Cut);
      instance.SetNewArray(&newArray_Cut);
      instance.SetDelete(&delete_Cut);
      instance.SetDeleteArray(&deleteArray_Cut);
      instance.SetDestructor(&destruct_Cut);
      return &instance;
   }
   TGenericClassInfo *GenerateInitInstance(const ::Cut*)
   {
      return GenerateInitInstanceLocal(static_cast<::Cut*>(nullptr));
   }
   // Static variable to force the class initialization
   static ::ROOT::TGenericClassInfo *_R__UNIQUE_DICT_(Init) = GenerateInitInstanceLocal(static_cast<const ::Cut*>(nullptr)); R__UseDummy(_R__UNIQUE_DICT_(Init));

   // Dictionary for non-ClassDef classes
   static TClass *Cut_Dictionary() {
      TClass* theClass =::ROOT::GenerateInitInstanceLocal(static_cast<const ::Cut*>(nullptr))->GetClass();
      Cut_TClassManip(theClass);
   return theClass;
   }
...
namespace ROOT {
   ...
   static void *new_Cut(void *p) {
      return  p ? new(p) ::Cut : new ::Cut;
   }
   static void *newArray_Cut(Long_t nElements, void *p) {
      return p ? new(p) ::Cut[nElements] : new ::Cut[nElements];
   }
   // Wrapper around operator delete
   static void delete_Cut(void *p) {
      delete (static_cast<::Cut*>(p));
   }
   static void deleteArray_Cut(void *p) {
      delete [] (static_cast<::Cut*>(p));
   }
   static void destruct_Cut(void *p) {
      typedef ::Cut current_t;
      (static_cast<current_t*>(p))->~current_t();
   }
} // end of namespace ROOT for class ::Cut
...
"/work/pjgwak/pol24/archive/code_structure_test_250523/tutoiral/10/",
nullptr
    };
...
class __attribute__((annotate("$clingAutoload$Cut.h")))  __attribute__((annotate("$clingAutoload$EventLoop.h")))  Cut;
