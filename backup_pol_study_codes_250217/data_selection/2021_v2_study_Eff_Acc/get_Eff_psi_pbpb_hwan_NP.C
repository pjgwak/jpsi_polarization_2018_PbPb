...
void get_Eff_psi_pbpb_hwan_NP(
  ...
  double centBin_for[7] = {0,20,40,60,80,100,180}; //230528
  ...
  //for(int iev=0; iev<300000 ; ++iev)
  {
    ...
    for(int igen = 0; igen<Gen_QQ_size; igen++){
	    ...
	    if( (Centrality > cLow && Centrality < cHigh)) { 
			if(Rapidity_g > 1.6 && Rapidity_g <2.4){ hpt_gen_1->Fill(JP_Gen->Pt(),weight*pt_weight); counts[5]++;}
			if(Rapidity_g < 1.6 ) hpt_gen_2->Fill(JP_Gen->Pt(),weight*pt_weight); }
		if(Rapidity_g > 1.6 && Rapidity_g <2.4 && JP_Gen->Pt() > 3.5 && JP_Gen->Pt() < 50){
    ...
    for (Int_t irqq=0; irqq<Reco_QQ_size; ++irqq) 
    {
      ...
      if(! (JP_Reco->Pt()>0&&JP_Reco->Pt()<50&&fabs(JP_Reco->Rapidity())<2.4&&IsAcceptanceQQ(mupl_Reco->Pt(),fabs(mupl_Reco->Eta()))&&IsAcceptanceQQ(mumi_Reco->Pt(),fabs(mumi_Reco->Eta()))) ) continue;
...
//         cout << "TnP ERROR !!!! ::: No matched L2 filter1 " << endl;
      ...
      if(HLTPass==true && HLTFilterPass==true){
		  ...
		  if(Rapidity > 1.6 && Rapidity <2.4 && JP_Reco->Pt() > 3.5 && JP_Reco->Pt() < 50){
