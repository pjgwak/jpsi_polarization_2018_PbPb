...
void eff_acc(string data_label_ = "test", int nevt = 10, int MCtype = 1, int cLow = 0, int cHigh = 180, bool isTnP = true, bool isPtWeight = true)
{
    ...
    vector<double> fwd_pt_bin = {0, 3, 4, 5, 6.5, 9, 12, 50};
    vector<double> mid_pt_bin = {6.5, 9, 12, 15, 20, 50};
    ...
    auto c0_20_fwd_num = std::make_shared<TH2D>("c0_20_fwd_num", "", n_cos, &cos_bin[0], n_fwd_pt, &fwd_pt_bin[0]);
    auto c0_20_fwd_den = std::make_shared<TH2D>("c0_20_fwd_den", "", n_cos, &cos_bin[0], n_fwd_pt, &fwd_pt_bin[0]);
    auto eff_c0_20_fwd = std::make_shared<TH2D>("eff_c0_20_fwd", "", n_cos, &cos_bin[0], n_fwd_pt, &fwd_pt_bin[0]);
    auto c0_20_mid_num = std::make_shared<TH2D>("c0_20_mid_num", "", n_cos, &cos_bin[0], n_mid_pt, &mid_pt_bin[0]);
    auto c0_20_mid_den = std::make_shared<TH2D>("c0_20_mid_den", "", n_cos, &cos_bin[0], n_mid_pt, &mid_pt_bin[0]);
    auto eff_c0_20_mid = std::make_shared<TH2D>("eff_c0_20_mid", "", n_cos, &cos_bin[0], n_mid_pt, &mid_pt_bin[0]);

    auto c20_60_fwd_num = std::make_shared<TH2D>("c20_60_fwd_num", "", n_cos, &cos_bin[0], n_fwd_pt, &fwd_pt_bin[0]);
    auto c20_60_fwd_den = std::make_shared<TH2D>("c20_60_fwd_den", "", n_cos, &cos_bin[0], n_fwd_pt, &fwd_pt_bin[0]);
    auto eff_c20_60_fwd = std::make_shared<TH2D>("eff_c20_60_fwd", "", n_cos, &cos_bin[0], n_fwd_pt, &fwd_pt_bin[0]);
    auto c20_60_mid_num = std::make_shared<TH2D>("c20_60_mid_num", "", n_cos, &cos_bin[0], n_mid_pt, &mid_pt_bin[0]);
    auto c20_60_mid_den = std::make_shared<TH2D>("c20_60_mid_den", "", n_cos, &cos_bin[0], n_mid_pt, &mid_pt_bin[0]);
    auto eff_c20_60_mid = std::make_shared<TH2D>("eff_c20_60_mid", "", n_cos, &cos_bin[0], n_mid_pt, &mid_pt_bin[0]);

    auto c60_180_fwd_num = std::make_shared<TH2D>("c60_180_fwd_num", "", n_cos, &cos_bin[0], n_fwd_pt, &fwd_pt_bin[0]);
    auto c60_180_fwd_den = std::make_shared<TH2D>("c60_180_fwd_den", "", n_cos, &cos_bin[0], n_fwd_pt, &fwd_pt_bin[0]);
    auto eff_c60_180_fwd = std::make_shared<TH2D>("eff_c60_180_fwd", "", n_cos, &cos_bin[0], n_fwd_pt, &fwd_pt_bin[0]);
    auto c60_180_mid_num = std::make_shared<TH2D>("c60_180_mid_num", "", n_cos, &cos_bin[0], n_mid_pt, &mid_pt_bin[0]);
    auto c60_180_mid_den = std::make_shared<TH2D>("c60_180_mid_den", "", n_cos, &cos_bin[0], n_mid_pt, &mid_pt_bin[0]);
    auto eff_c60_180_mid = std::make_shared<TH2D>("eff_c60_180_mid", "", n_cos, &cos_bin[0], n_mid_pt, &mid_pt_bin[0]);

    // =====  make hist. save error ===== //
    c0_20_fwd_num ->Sumw2();
    c0_20_fwd_den->Sumw2();
    c0_20_mid_num->Sumw2();
    c0_20_mid_den->Sumw2();

    c20_60_fwd_num->Sumw2();
    c20_60_fwd_den->Sumw2();
    c20_60_mid_num->Sumw2();
    c20_60_mid_den->Sumw2();

    c60_180_fwd_num->Sumw2();
    c60_180_fwd_den->Sumw2();
    c60_180_mid_num->Sumw2();
    c60_180_mid_den->Sumw2();

    eff_c0_20_fwd->Sumw2();
    eff_c0_20_mid->Sumw2();
    eff_c20_60_fwd->Sumw2();
    eff_c20_60_mid->Sumw2();
    eff_c60_180_fwd->Sumw2();
    eff_c60_180_mid->Sumw2();
    ...
    for (int iev = 0; iev < nevt; ++iev)
    {
        ...
        for (int igen = 0; igen < Gen_QQ_size; igen++)
        {
            ...
            {
                if (Rapidity_gen > 1.6)
                    c0_20_fwd_den->Fill(cos_ep_, JP_Gen->Pt(), weight * pt_weight);
                else
                    c0_20_mid_den->Fill(cos_ep_, JP_Gen->Pt(), weight * pt_weight);
            }

            if (Centrality > 20 && Centrality < 60 && JP_Gen->Pt() > 3)
            {
                if (Rapidity_gen > 1.6)
                    c20_60_fwd_den->Fill(cos_ep_, JP_Gen->Pt(), weight * pt_weight);
                else
                    c20_60_mid_den->Fill(cos_ep_, JP_Gen->Pt(), weight * pt_weight);
            }

            if (Centrality > 60 && Centrality < 180 && JP_Gen->Pt() > 3)
            {
                if (Rapidity_gen > 1.6)
                    c60_180_fwd_den->Fill(cos_ep_, JP_Gen->Pt(), weight * pt_weight);
                else
                    c60_180_mid_den->Fill(cos_ep_, JP_Gen->Pt(), weight * pt_weight);
            }
        } // end of gen loop
        ...
        for (Int_t irqq = 0; irqq < Reco_QQ_size; ++irqq)
        {
            ...
            if (Centrality > 0 && Centrality < 20 && JP_Reco->Pt() > 3)
            {
                if (Rapidity_reco > 1.6)
                    c0_20_fwd_num->Fill(cos_ep_, JP_Reco->Pt(), weight * tnp_weight * pt_weight);
                else
                    c0_20_mid_num->Fill(cos_ep_, JP_Reco->Pt(), weight * tnp_weight * pt_weight);
            }

            if (Centrality > 20 && Centrality < 60 && JP_Reco->Pt() > 3)
            {
                if (Rapidity_reco > 1.6)
                    c20_60_fwd_num->Fill(cos_ep_, JP_Reco->Pt(), weight * tnp_weight * pt_weight);
                else
                    c20_60_mid_num->Fill(cos_ep_, JP_Reco->Pt(), weight * tnp_weight * pt_weight);
            }

            if (Centrality > 60 && Centrality < 180 && JP_Reco->Pt() > 3)
            {
                if (Rapidity_reco > 1.6)
                    c60_180_fwd_num->Fill(cos_ep_, JP_Reco->Pt(), weight * tnp_weight * pt_weight);
                else
                    c60_180_mid_num->Fill(cos_ep_, JP_Reco->Pt(), weight * tnp_weight * pt_weight);
            }
        } // end of reco loop
    ...
    divide_2d_hist(c0_20_fwd_num, c0_20_fwd_den, eff_c0_20_fwd);
    divide_2d_hist(c0_20_mid_num, c0_20_mid_den, eff_c0_20_mid);
    divide_2d_hist(c20_60_fwd_num, c20_60_fwd_den, eff_c20_60_fwd);
    divide_2d_hist(c20_60_mid_num, c20_60_mid_den, eff_c20_60_mid);
    divide_2d_hist(c60_180_fwd_num, c60_180_fwd_den, eff_c60_180_fwd);
    divide_2d_hist(c60_180_mid_num, c60_180_mid_den, eff_c60_180_mid);
    ...
    //"|y|: 0.0-0.6, 0-10%"
    draw_2d_hist(eff_c0_20_fwd, "|y|: 1.6-2.4, 0-10%", MCtype);
    draw_2d_hist(eff_c0_20_mid, "|y|: 0.0-1.6, 0-10%", MCtype);
    draw_2d_hist(eff_c20_60_fwd, "|y|: 1.6-2.4, 10-30%", MCtype);
    draw_2d_hist(eff_c20_60_mid, "|y|: 0.0-1.6, 10-30%", MCtype);
    draw_2d_hist(eff_c60_180_fwd, "|y|: 1.6-2.4, 60-90%", MCtype);
    draw_2d_hist(eff_c60_180_mid, "|y|: 0.0-1.6, 60-90%", MCtype);

    eff_c0_20_fwd->SetName(Form("mc_eff_vs_pt_TnP%d_PtW%d_cent0to20_absy1p6_2p4", isTnP, isPtWeight));
    eff_c0_20_mid->SetName(Form("mc_eff_vs_pt_TnP%d_PtW%d_cent0to20_absy0_1p6", isTnP, isPtWeight));
    eff_c20_60_fwd->SetName(Form("mc_eff_vs_pt_TnP%d_PtW%d_cent20_60_absy1p6_2p4", isTnP, isPtWeight));
    eff_c20_60_mid->SetName(Form("mc_eff_vs_pt_TnP%d_PtW%d_cent20_60_absy0_1p6", isTnP, isPtWeight));
    eff_c60_180_fwd->SetName(Form("mc_eff_vs_pt_TnP%d_PtW%d_cent60to180_absy1p6_2p4", isTnP, isPtWeight));
    eff_c60_180_mid->SetName(Form("mc_eff_vs_pt_TnP%d_PtW%d_cent60to180_absy0_1p6", isTnP, isPtWeight));


    // ===== save output ===== //
    TString outFileName = Form("mc_eff_vs_pt_cent_rap_prompt_pbpb_Jpsi_PtW%d_tnp%d_%s.root", isPtWeight, isTnP, date_label.Data());
    if (MCtype == 2)
        outFileName = Form("mc_eff_vs_pt_cent_rap_nprompt_pbpb_Jpsi_PtW%d_tnp%d_%s.root", isPtWeight, isTnP, date_label.Data());
    TFile *outFile = new TFile(outFileName, "RECREATE");
    
    eff_c0_20_fwd->Write();
    eff_c0_20_mid->Write();
    eff_c20_60_fwd->Write();
    eff_c20_60_mid->Write();
    eff_c60_180_fwd->Write();
    eff_c60_180_mid->Write();
    // f1->Write();
    if (isTnP)
